# java-interview-min-stack-starter-kit

Starter kit for Java Min Stack problem.
...
- You can click on the `Run I/O` button from the interview application, which will run your solution against some preconfigured inputs and show you the output.
...
- To run, issue the command `java -jar dist/lib/dist.jar '<input 1>'` where
  ...
  For example: `java -jar dist/lib/dist.jar '["MinStack","push","push","push","getMin","pop","top","getMin"] | [[],[-2],[0],[-3],[],[],[],[]]'
` <br>
...
- `run.sh '<input 1>' '<input 2>'` - compiles and runs the code against the given inputs. Example: `run.sh '["MinStack","push","push","push","getMin","pop","top","getMin"] | [[],[-2],[0],[-3],[],[],[],[]]'
`.
